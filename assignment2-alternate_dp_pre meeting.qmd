...
covid_wide <- spread(covid, key= geo_value, value = value)
...
#This creates an indicator for the counties in the covid dataset (and renames to match the ACS)
covid_county <- covid %>% 
  select(geo_value) %>% 
  mutate(covid=1) %>% 
  rename(location=geo_value)

#This creates an indicator for the counties in the ACS dataset
acs_county <- acs %>% 
  select(location) %>% 
  mutate(acs=1)

#Merging the two together to see which ones do not appear in both data sets
mergeStatus <- merge(covid_county,acs_county, all=T) %>%
  mutate(notmerged=ifelse(is.na(covid)==T | is.na(acs)==T,1,0))
table(mergeStatus$notmerged)

###Now creating a new data set by joining the two data sets, keeping only counties that appear in both data sets###

acs <- acs %>% 
  rename(geo_value = location) #first need to rename so that we can merge on them

acs_covid <- inner_join(
  covid,
  acs,
  by = "geo_value"
)
...
NOTE: I DON'T THINK THERE IS ANY DATA FOR APRIL 1ST? 

Below, we can see that the mean proportion of people with COVID like illness symptoms on April 6, 2020 was .88 in above median household income counties, and 1.0 in counties below the median household income. Therefore, we can draw the conclusion that there may be an association between income level and propotion of people with COVID like symtpons - that it might be more common in low income suburbs. 


```{r}

#first, showing there are no april 1, 2020 values in the dataset, so below I'll use april 6th instead
table(acs_covid$time_value)

acs_covid %>%
  mutate (mean = mean(acs_covid$income))%>%
  mutate (income_mean = ifelse(acs_covid$income > mean, "Above mean HH income", "Below mean HH income")) %>%
  group_by(income_mean) %>%
  filter(time_value == 	ymd("2020-04-06")) %>%
    summarize(mean = mean(value)) 
...
In general, higher incomes appear associated with somewhat lower proportions. 
...
acs_covid %>%
  ...
  labs(x = "Median household income",
       y = "Proportion with COVID-like illness",
       color = "Income group")
